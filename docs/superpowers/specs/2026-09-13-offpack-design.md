...
- Команды pnpm, yarn, bun на входе (выполняются как нераспознанные), Docker-образы. pnpm используется только внутри песочницы для npm-прохода.
...
- Пока идут проходы, в фоне читается `docker compose logs -f --no-color --no-log-prefix verdaccio devpi`. По строке на каждый скачанный файл пакета (без повторов за сборку): `↓ npm  <имя> <версия>  <размер>` или `↓ pypi <имя файла>`. Метаданные (packument, simple-страницы), HEAD и ответы не 200 не считаются.
  - Verdaccio 6 (`level: http`): `http <-- 200, user: null(172.19.0.5), req: 'GET /@esbuild/win32-x64/-/win32-x64-0.28.2.tgz', bytes: 0/4829680`. Каждый запрос пишется дважды — по закрытию запроса (обычно `bytes: 0/0`) и по окончании ответа (итоговый размер); учитываются строки с ненулевым размером.
  - devpi-server 6 (уровень INFO по умолчанию): `[req10] GET /root/pypi/+f/815/e7be7a7806d54/idna-3.19-py3-none-any.whl` — пишется в начале запроса, размера нет, поэтому PyPI-файлы печатаются без размера.
...
- Фоновое чтение логов всегда останавливается, стек всегда удаляется в `finally`.
