...
- Строки `↓` — по логам прокси, каждый файл один раз: `npm` — tarball, которые Verdaccio отдал
  (с размером), `pypi` — запросы файлов к devpi (devpi пишет запрос в лог до ответа и без размера,
  поэтому строка не гарантирует, что файл скачан). Это только ход сборки: что попало в архив,
  показывают итог и отчёт. Управляющие символы в выводе экранируются (`\x1b`), так как пути
  запросов и вывод установочных скриптов задаёт код из песочницы.
...
по URL и локальным путям, команды pnpm и yarn (выполняются как есть, только под платформу
песочницы), bun (в песочнице его нет), платформы arm64 и macOS, сканирование уязвимостей,
перенос бинарников, которые `postinstall` скачивает не из реестров.

Если у npm-пакета есть транзитивная зависимость из git или по URL, сборка завершается ошибкой:
pnpm в песочнице по умолчанию запрещает такие зависимости (`block-exotic-subdeps`).
